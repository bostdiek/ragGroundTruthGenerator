...
async def test_qa_pair_endpoints():
    async with httpx.AsyncClient() as client:
        ...
        print(f"QA pair retrieved successfully")
        ...
        response = await client.patch(
            f"{BASE_URL}/qa-pairs/{qa_pair_id}",
            json=update_data
        )
        ...
        print(f"QA pair updated successfully")
        ...
        if verified_qa_pair["status"] == new_status:
            ...
            print(f"Update failed. Expected {new_status}, got {verified_qa_pair['status']}")
