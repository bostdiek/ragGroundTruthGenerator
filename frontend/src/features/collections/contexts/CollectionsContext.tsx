...
import CollectionsService, { Collection, QAPair } from '../../collections/api/collections.service';
...
interface CollectionsContextType {
  ...
  createQAPair: (collectionId: string, qaPairData: Partial<QAPair>) => Promise<QAPair>;
}
...
export const CollectionsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchQAPairs = async (collectionId: string) => {
    ...
    try {
      ...
      console.error(`Failed to fetch Q&A pairs for collection ${collectionId}:`, err);
  ...
  const createCollection = async (name: string, description: string) => {
    ...
    try {
      const newCollection = await CollectionsService.createCollection({ name, description });
  ...
  const updateCollection = async (id: string, name: string, description: string) => {
    ...
    try {
      const updatedCollection = await CollectionsService.updateCollection(id, { name, description });
  ...
  const createQAPair = async (collectionId: string, qaPairData: Partial<QAPair>) => {
    setIsLoading(true);
    setError(null);
    
    try {
      const newQAPair = await CollectionsService.createQAPair(collectionId, qaPairData);
      ...
      console.error(`Failed to create QA pair for collection ${collectionId}:`, err);
