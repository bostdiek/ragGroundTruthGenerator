...
describe('AuthService', () => {
  ...
  describe('login', () => {
    it('should make a POST request to /auth/login with credentials', async () => {
      ...
      expect(localStorage.setItem).toHaveBeenCalledWith('auth_token', mockTokenResponse.access_token);
      expect(localStorage.setItem).toHaveBeenCalledWith('auth_user', JSON.stringify(mockTokenResponse.user));
    });
